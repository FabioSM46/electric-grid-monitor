...
func main() {
	...
	// Persist kwh_cost to the DB so Grafana SQL queries can reference it.
	if err := store.SaveSetting("kwh_cost", strconv.FormatFloat(cfg.KwhCost, 'f', -1, 64)); err != nil {
		log.Printf("Warning: failed to save kwh_cost setting: %v", err)
	}
	...
	// Start API server (blocks until context is cancelled)
...
func runCollector(ctx context.Context, cfg *config.Config, coll *collector.Collector, store *storage.Store) {
	ticker := time.NewTicker(cfg.CollectionInterval.Duration)
	defer ticker.Stop()
